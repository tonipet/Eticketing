...
function DialoggetSubTicketType() {

    var empObj = {
        TicketTypeCode: $('#DialogStickertype').val()
    };
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#DialogSubStickertype').empty();
            $('#DialogSubStickertype')
            $('#DialogSubStickertype').append('<option selected="" value=""></option>')
            $.each(result, function (val, text) {
            ...
            $('#DialogSubStickertype').select2().select2('val', [GlobalSubticket]);
            GlobalSubticket = "";
        },
        error: function (errormessage) {
            //alert(errormessage.responseText);
...
function Dialoggetstickertype() {
    $.ajax({
        ...
        dataType: "json",
        success: function (result) {
            $('#DialogStickertype').append('<option selected="" value=""></option>')
            $.each(result, function (val, text) {
                $('#DialogStickertype')
                     .append(new Option(this['Text'], this['Value'])
                 );
...
function ticketModal(id) {
    ...
    setTimeout(function () {
        ...
        $('.direct-chat-messages').animate({ scrollTop: 10000000 });
    }, 2000);
...
function getBYID(id) {
    $.ajax({
        ...
        success: function (result) {
            ...
            GlobalSubticket = result.SubTicketType;
            $('#DialogStickertype').select2().select2('val', [result.TicketTypeCode]);
            $("#Dialoginformation").text("Title:" + result.Title + "/" + "Department:" + result.DepartmentName + "/" );
      
            $('#DialogAssignto').select2().select2('val', [result.AssignToCode]);
            if (result.TargetCompletionDate == "") {
...
function addTicketDialog() {
    ...
    var empObj = {
        ...
        AttachmentFile: ' ',
        TicketTypeCode: $('#DialogStickertype').val(),
        SubTicketType: $('#DialogSubStickertype').val()
    };
...
$(document).on('click', '#TEST234', function () {
...
function DialogValidate() {
    ...
    if ($('#DialogSubStickertype').val() == null || $('#DialogSubStickertype').val() == '') {
        $("#DialogSubTicketTypeErr").html("<small>* required </small>");
        isvalid = false;
    }

    if ($('#DialogStickertype').val() == null) {
        $("#DialogStickerTypeErr").html("<small>* required </small>");
        isvalid = false;
    }
    return isvalid;
}
...
function disableDialog() {
    $('#DialogStatus').select2("enable", false);
    $('#DialogAssignto').select2("enable", false);
    $('#DialogTags').select2("enable", false);
    $('#saveDialogSave').hide();
    $('#DialogSubStickertype').select2("enable", false);
    $('#DialogStickertype').select2("enable", false);
    $("#dialogMsg").prop('disabled', true);
    $("#datepicker").prop('disabled', true);
   
}

function EnableDialog() {
    $('#DialogStatus').prop('disabled', false);
    $('#DialogSubStickertype').prop('disabled', false);
    $('#DialogStickertype').prop('disabled', false);
    $("#dialogMsg").prop('disabled', false);
    $('#DialogStatus').prop('disabled', false);
    $('#DialogAssignto').prop('disabled', false);
    $('#DialogTags').prop('disabled', false);
    $('#saveDialogSave').show();
